...
func (cs *CoinSet) TotalValue() (value aibutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue aibutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue aibutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount aibutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue aibutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue aibutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue aibutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue aibutil.Amount, coins []Coin) (Coins, error) {
...
// aibutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() aibutil.Amount {
	return aibutil.Amount(c.txOut().Value)
}
