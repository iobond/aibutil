...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := aibutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(aibutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := aibutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := aibutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := aibutil.NewBlockFromBytes(blockBytes)
	if err != nil {
