...
/*
Package aibutil provides AIB-specific convenience functions and types.

Block Overview

A Block defines a AIB block that provides easier and more efficient
...
A Tx defines a AIB transaction that provides more efficient manipulation of
...
The Address interface provides an abstraction for a AIB address.  While the
most common type is a pay-to-pubkey-hash, AIB already supports others and
...
To decode/encode an address:
	...
	addr, err := aibutil.DecodeAddress(addrString, defaultNet)
	if err != nil {
		fmt.Println(err)
		return
	}
